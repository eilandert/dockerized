...
process_template "$TEMPLATE" "Dockerfile-multi-ubu" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_PHP_FPM}:multi"

log_info "  Nginx multi (debian)"
process_template "$TEMPLATE" "Dockerfile-multi-deb" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_PHP_FPM}:deb-multi"

# Base: ubuntu rolling (no PHP)
log_info "  Nginx base (ubuntu)"
process_template "$TEMPLATE" "Dockerfile-ubu" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_UBUNTU_BASE}:${UBUNTU_BASE_TAG}"

log_info "  Nginx base (debian)"
process_template "$TEMPLATE" "Dockerfile-deb" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_DEBIAN_BASE}:${DEBIAN_BASE_TAG}"
...
for version in "${PHP_VERSIONS[@]}"; do
    ...
    process_template "$TEMPLATE" "$ubuntu_output" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_PHP_FPM}:${version}"
    
    log_info "  Nginx PHP $version (debian)"
    process_template "$TEMPLATE" "$debian_output" "FROM=${DOCKER_REGISTRY_PREFIX}/${IMAGE_PREFIX_PHP_FPM}:deb-${version}"
done
